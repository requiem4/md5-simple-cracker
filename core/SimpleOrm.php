...
abstract class SimpleOrm
{
     ...
     * @return void
     */
    public static function useConnection ($conn, $database)
    {
        self::$conn = $conn;
    ...
    public static function all($params = [])
    {
        ...
        return self::sql("SELECT * FROM `:database`.:table " . $order);
    }
    ...
    public static function objectsToArray($objects){
    	$array = [];
			foreach ($objects as $object) {
				$array[] = self::convertObjectToArray($object);
    	}
			return $array;
		}
	public static function allAsArray($fields = []) {
		if(empty($fields)){
			$fields = ['*'];
		}
		$properties = implode(',' ,$fields);
		$properties = trim($properties,',');
    $sql = "SELECT " . $properties . " FROM `:database`.:table";
		$sql = str_replace(array(':database', ':table', ':pk'), array(self::getDatabaseName(), self::getTableName(), self::getTablePk()), $sql);
